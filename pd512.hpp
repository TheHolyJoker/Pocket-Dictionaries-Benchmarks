...
namespace pd512 {
    ...
    inline uint64_t blrsn_pdep(uint64_t a, uint64_t n) {
        uint64_t mask = ~_bzhi_u64(-1, n + 1);
        a = _pdep_u64(mask, a);
        return a;
    }
    ...
    inline bool pd_find_50_x2(int64_t quot, uint8_t rem, const __m512i *pd) {
        assert(false);
        assert(0 == (reinterpret_cast<uintptr_t>(pd) % 64));
        assert(quot < 50);
        const __m512i target = _mm512_set1_epi8(rem);
        ...
        assert(popcount128(header) == 50);
        const int64_t pop = _mm_popcnt_u64(header);
        // this expression is too complex, but this might work.
        const uint64_t begin = (quot ? (select128withPop64(header, quot - 1, pop) + 1) : 0) - quot;
        const uint64_t clear_up_to_begin = (header >> begin) << begin;
        const uint64_t mask = _blsmsk_u64((header >> begin) << begin) >> (1);
        const uint64_t end = select128withPop64(header, quot, pop) - quot;
        const uint64_t valid_mask = ((UINT64_C(1) << end) - 1);
        assert(mask == valid_mask);
        bool res = (v & mask) >> begin;
    ...
    inline bool pd_find_50_v3(int64_t quot, uint8_t rem, const __m512i *pd) {
        ...
        if (quot == 0) {
            ...
            const uint64_t temp = _tzcnt_u64(h1) + 1;
            const uint64_t diff = _tzcnt_u64(h1 >> temp);
            const bool a = diff && ((v >> (temp - quot)) & ((UINT64_C(1) << diff) - 1));
            // const bool b = diff && ((v << (quot - temp)) & ((UINT64_C(1) << diff) - 1));
            // assert(a == b);
            // return diff && ((v >> (temp - quot)) & ((UINT64_C(1) << diff) - 1));
            // const uint64_t my_begin = 64 - quot + temp;
            // const uint64_t my_end = my_begin + _tzcnt_u64(h1 >> temp);
            ...
            //     std::cout << "a:\t\t" << v_pd512::bin_print_header_spaced(a) << std::endl;
            //     std::cout << "b:\t\t" << v_pd512::bin_print_header_spaced(b) << std::endl;
            //     std::cout << "r_shift:\t\t" << v_pd512::bin_print_header_spaced(v >> (temp - quot)) << std::endl;
            //     std::cout << "l_shift:\t\t" << v_pd512::bin_print_header_spaced(v << (quot - temp)) << std::endl;
            //     std::cout << "\t\t" << std::string(80, '-') << std::endl;
            //     assert(0);
            // }
            // // assert(a == b);
    ...
    inline bool pd_find_50_v11(int64_t quot, uint8_t rem, const __m512i *pd) {
        ...
        if (quot == 0) {
            ...
            const uint64_t end = _tzcnt_u64(h0);
            return (end) && (v & ((UINT64_C(1) << end) - 1));
            // return (end) && (v & (_blsmsk_u64(h0 >> 1ul)));
            // bool res = (end) && (v & ((UINT64_C(1) << end) - 1));
            // assert(res == pd_find_50_v1(quot, rem, pd));
    ...
    inline bool pd_find_50_v12(int64_t quot, uint8_t rem, const __m512i *pd) {
        ...
        if ((_blsr_u64(v) == 0)) {
            ...
            const unsigned __int128 mask = ((unsigned __int128) v) << quot;
            assert(mask >= v);
            return (!((mask & header) == mask)) && (popcount128(header & (mask - 1)) == quot);
            return popcount128(header & (mask - 1)) == quot;
        }
        ...
        // const unsigned __int128 mask = ((const unsigned __int128) v) << quot;
        // assert(mask >= v);
        // if ((mask & header) == mask) return false;
        //
        // const uint64_t h0 = ((uint64_t *) pd)[0];
        // const uint64_t h1 = ((uint64_t *) pd)[1];
        //
        // return (v << quot) ?  _mm_popcnt_u64(h0 & (mask - 1)) == quot : popcount128(header & (mask - 1)) == quot;
        // return ((v << quot) ?  _mm_popcnt_u64(header & (mask - 1)) : popcount128(header & (mask - 1)) ) == quot;
        // if (v << quot) {
        //     // const unsigned __int128 *h = (const unsigned __int128 *) pd;
        //     // const unsigned __int128 header = (*h);
        //     const int64_t mask = v << quot;
    ...
    inline bool pd_find_50_v13(int64_t quot, uint8_t rem, const __m512i *pd) {
        assert(0 == (reinterpret_cast<uintptr_t>(pd) % 64));
        assert(quot < 50);
        const __m512i target = _mm512_set1_epi8(rem);
        const uint64_t v = _mm512_cmpeq_epu8_mask(target, *pd) >> 13ul;
    ...
    inline bool pd_find_50_v14(int64_t quot, uint8_t rem, const __m512i *pd) {
        assert(0 == (reinterpret_cast<uintptr_t>(pd) % 64));
        assert(quot < 50);
        const __m512i target = _mm512_set1_epi8(rem);
        uint64_t v = _mm512_cmpeq_epu8_mask(target, *pd) >> 13ul;
    ...
    inline bool pd_find_50_v15_validation(int64_t quot, uint8_t rem, const __m512i *pd) {
        assert(0 == (reinterpret_cast<uintptr_t>(pd) % 64));
        assert(quot < 50);
        const __m512i target = _mm512_set1_epi8(rem);
        uint64_t v = _mm512_cmpeq_epu8_mask(target, *pd) >> 13ul;
        ...
        // / const int64_t pop = _mm_popcnt_u64(header);
        const uint64_t begin = select128withPop64(header, quot - 1, pop) + 1 - quot;
        const uint64_t end = select128withPop64(header, quot, pop) - quot;
        ...
        if (quot < pop - 1) {
            ...
            // uint64_t mask = ~(_bzhi_u64(-1, quot) >> quot);
            uint64_t mask = ~(_bzhi_u64(-1, quot));
            ...
            if (b_att == res)
                return res;
            std::cout << "header:\t\t" << v_pd512::bin_print_header_spaced(header) << std::endl;
            std::cout << "v:\t\t" << v_pd512::bin_print_header_spaced(v) << std::endl;
            std::cout << "mask_m1\t\t" << v_pd512::bin_print_header_spaced(_pdep_u64(mask >> 1ul, h0)) << std::endl;
            std::cout << "mask:\t\t" << v_pd512::bin_print_header_spaced(_pdep_u64(mask, h0)) << std::endl;
            std::cout << "mask_m1\t\t" << v_pd512::bin_print_header_spaced(_pdep_u64(mask >> 1ul, h0) - 1) << std::endl;
            ...
            // std::cout<< k++ << ":\t" << v_pd512::bin_print_header_spaced(_pdep_u64((mask << 1) | 1, h0)) << std::endl;
            // std::cout<< k++ << ":\t" << v_pd512::bin_print_header_spaced(_bzhi_u64(-1, quot)) << std::endl;
            // std::cout<< k++ << ":\t" << v_pd512::bin_print_header_spaced(_bzhi_u64(-1, quot + 1)) << std::endl;
            // std::cout<< k++ << ":\t" << v_pd512::bin_print_header_spaced(~_bzhi_u64(-1, quot)) << std::endl;
            // std::cout<< k++ << ":\t" << v_pd512::bin_print_header_spaced(mask) << std::endl;
            std::cout << k++ << ":\t" << v_pd512::bin_print_header_spaced(begin_mask) << std::endl;
            std::cout << k++ << ":\t" << v_pd512::bin_print_header_spaced(begin_mask_shifted) << std::endl;
            std::cout << k++ << ":\t" << std::string(80, '!') << std::endl;
            uint64_t v_temp = (v & ((UINT64_C(1) << end) - 1));
            uint64_t a = (v_temp >> begin);
            uint64_t b = (v_temp & begin_mask);
            uint64_t c = (v_temp & begin_mask_shifted);
            // assert((b == a) || (c == a));
            // assert(c == a);
            assert(b == a);

            // assert((v_temp >> begin) == (v_temp & begin_mask_shifted));
            // uint64_t val_pdep_res_begin = _pdep_u64(mask >> 1ul, (uint64_t) header);
            uint64_t pdep_res_end = _pdep_u64(mask, h0);
            uint64_t end_mask = _blsmsk_u64(pdep_res_end);
            uint64_t end_mask_shifted = _blsmsk_u64(pdep_res_begin) >> quot;
            assert(v_temp == (v & end_mask));
            bool att = v & begin_mask & end_mask;
            assert(att == res);
            return att;
            // return v & begin_mask & end_mask;

            // assert
            // uint64_t end_mask_mid = _pdep_u64(mask, quot);
            // uint64_t end_mask = _pdep_u64(mask, quot);
            // bool att = (v & ((UINT64_C(1) << end) - 1)) >> begin;
            // return diff && ((v >> (temp - quot)) & ((UINT64_C(1) << diff) - 1));
    ...
    inline bool pd_find_50_v15(int64_t quot, uint8_t rem, const __m512i *pd) {
        assert(0 == (reinterpret_cast<uintptr_t>(pd) % 64));
        assert(quot < 50);
        const __m512i target = _mm512_set1_epi8(rem);
        uint64_t v = _mm512_cmpeq_epu8_mask(target, *pd) >> 13ul;
    ...
    inline bool pd_find_50_v16(int64_t quot, uint8_t rem, const __m512i *pd) {
        assert(0 == (reinterpret_cast<uintptr_t>(pd) % 64));
        assert(quot < 50);
        const __m512i target = _mm512_set1_epi8(rem);
        uint64_t v = _mm512_cmpeq_epu8_mask(target, *pd) >> 13ul;
    ...
    inline bool pd_find_50(int64_t quot, uint8_t rem, const __m512i *pd) {
        ...
        assert(pd_find_50_v1(quot, rem, pd) == pd_find_50_v13(quot, rem, pd));
    ...
    inline bool pd_add_50(int64_t quot, char rem, __m512i *pd) {
        ...
        assert((fill <= 14) || (fill == ((unsigned) pd_popcount(pd))));
    ...
    inline auto is_full(const __m512i *x) -> bool {
        assert(get_capacity_naive(x) == ((size_t) get_capacity(x)));
